...
int miniPianoNumber; //numero di minipiano corrente
int cleanlinessTolerance=3;
...
std::vector<int> new_receivedNumbers; //Caso splitting del piano
...
//Inviare messaggio di fine trasmissione per il calcolo nel nuovo piano. 
...
void notifyServer() {
  ...
  // Costruisci la richiesta HTTP includendo il numero di minipiano come parametro nell'URL
  String request = "GET /capture?_cb=" + String(millis()) + "&mini_piano_number=" + String(miniPianoNumber) + "&current_position=" + String(current_position) + " HTTP/1.1\r\n";
  request.concat("Host: " + String(serverAddress) + "\r\n\r\n");
  ...
  if (client.available()) {
    ...
    // Controlla la risposta del server e chiama il metodo appropriato
    if (response.indexOf("dirty") != -1) {
      handleDirtyResponse();

    } else if (response.indexOf("clean") != -1) {
      //handleCleanResponse();
      
    } else {
      // Gestisci altri casi di risposta
    }
    if (response.indexOf("SPLIT_WORK") != -1) {
        // Se un robot richiede di collaborare
        int index = response.indexOf("SPLIT_WORK") + strlen("SPLIT_WORK: ");
        String subString = response.substring(index);
        
        // Ora subString contiene il vettore come stringa
        // Dovresti convertire questa stringa in un array di interi
        // Possiamo utilizzare strtok per estrarre i singoli numeri
        char *ptr = strtok(const_cast<char*>(subString.c_str()), " "); // Dividi la stringa in base agli spazi
        new_receivedNumbers.clear();
        while (ptr != NULL) {
            new_receivedNumbers.push_back(std::stoi(ptr));
            ptr = strtok(NULL, " ");
        }      
...
void getVectorFromServer() {
  ...
  if (client.available()) {
    ...
    if (vectorStart != -1 && vectorEnd != -1) {
      ...
      int miniPianoNumberStart = response.indexOf("mini_piano_number\":") + 19;
      ...
      miniPianoNumber = miniPianoNumberString.toInt();
...
void handleDirtyResponse(){
  customRobot.step_forward();
  for (int i=0;i<cleanlinessTolerance;i++){
    Buzzer_Alert(3,1);
    delay(1000);
